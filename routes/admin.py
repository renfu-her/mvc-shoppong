...
def dashboard():
    ...
    order_rows = order_rows.group_by(func.date(Order.created_at))
    order_rows = order_rows.order_by(func.date(Order.created_at)).all()

    order_map = {row.order_date: {"orders": row.order_count or 0, "revenue": float(row.order_total or 0)} for row in order_rows}
    order_chart_data = []
    ...
    product_rows = product_rows.join(Order, Order.id == OrderItem.order_id)
    product_rows = product_rows.filter(Order.created_at >= start_date)
    product_rows = product_rows.filter(Order.status.notin_(['cancelled', 'refunded', 'failed']))
    product_rows = product_rows.group_by(Product.id, Product.name)
    product_rows = product_rows.order_by(func.sum(OrderItem.quantity).desc())
    product_rows = product_rows.limit(10).all()
